...
exports.getOutfit = void 0;
const getOutfit = (filename) => {
    // 从给定的文件路径中提取番号（DVD ID）
    // 通常是接收文件的路径，当然如果是普通字符串也可以
    // var filename = path.basename(filepath);
    // filename = cfg.MovieID.ignore_pattern.replace("", filename);
    if ((filename === null || filename === void 0 ? void 0 : filename.toUpperCase().indexOf("ADD")) > -1) {
    ...
    if (filename_lc.includes("fc2")) {
        ...
        const match = filename.match(/fc2[^a-z\d]{0,5}(ppv[^a-z\d]{0,5})?(\d{5,7})/i);
        if (match) {
            return "FC2-" + match[2];
        }
    }
        ...
        if (no_domain !== filename) {
            const avid = (0, exports.getOutfit)(no_domain);
            if (avid) {
                return avid;
            }
        }
    ...
    if (filename.includes(")(")) {
        const avid = (0, exports.getOutfit)(filename.replace(/\)\(/g, "-"));
        if (avid) {
            return avid;
        }
    }
